...
def get_model(arch, config, model_count, device):
    if arch == "mlp":
        ...
        kwargs = {"N": config['model.rnn.N'], "H_in": config['model.rnn.H_in'], "H_out": config['model.rnn.H_out'],
                  ...
                  "num_of_rnn_layers": config['model.rnn.num_of_rnn_layers'], "model_count": model_count,
...
if __name__ == "__main__":
    ...
    model_name = get_model_name(config) + "_same_seeds"
        ...
        for cur_loss_bin in loss_bins:
            ...
            while True:
                if data_seed_is_not_good:
                    ...
                    print(f"DEBUG: train_data.shape={train_data.shape}  test_data.shape={test_data.shape} train_labels[0]={train_labels[0]} train_labels[1]={train_labels[1]}")
                ...
                if data_seed_is_not_good:
                    ...
                    for k in perfect_model_weights[0].keys():
                        if config['model.arch'] in ["mlp", "linear", "lenet"]:
                            good_models_state_dict[k] = torch.cat(
                                [weights_dictionary[k].cpu() for weights_dictionary in perfect_model_weights], dim=cat_dim)
                        else:
                            good_models_state_dict[k] = np.concatenate(
                                [weights_dictionary[k] for weights_dictionary in perfect_model_weights], axis=cat_dim)
                    ...
                    with torch.no_grad():
                        ...
                        test_loss, test_acc, _ = calculate_loss_acc(test_all_data.cpu(), test_all_labels.cpu(), new_models.forward, loss_func, batch_size=1)  # we don't use forward_normalize here because we are only using test accuracy, where normalization doesn't have any impact. ??? test_acc.shape=1 ???
    ...
    output_str += f"Experiment total time={time.time() - program_start_time}" + "\n"
