...
class LeNetModels(nn.Module):
    ...
    def forward_normalize(self, x):
        x = self.forward(x)
        print(f"DEBUG: forward_normalize: x.shape={x.shape}")
        print(f"DEBUG: forward_normalize: Example0_Model0_output={x[0,0,:]}")
        print(f"DEBUG: forward_normalize: Example0_Model1_output={x[0,1,:]}")
        cum_norm = 1
        for layer in [self.conv1, self.conv2, self.fc1, self.fc2, self.fc3]:
            cur_weight = layer.weight
            original_shape = cur_weight.shape
            cur_weight = cur_weight.view(self.model_count, -1, *original_shape[2:])
            print(f"DEBUG: forward_normalize: cur_weight.shape={cur_weight.shape}")
            cur_norm = cur_weight.norm(p=2, dim=(1, 2, 3), keepdim=True) /3
            cum_norm *= cur_norm.view(self.model_count, -1)
        x /= cum_norm
        print(f"DEBUG: forward_normalize: Model0_cum_norm={cum_norm[0,:]}")
        print(f"DEBUG: forward_normalize: Model1_cum_norm={cum_norm[1,:]}")
        print(f"DEBUG: forward_normalize: Example0_Model0_normalized_output={x[0,0,:]}")
        print(f"DEBUG: forward_normalize: Example0_Model1_normalized_output={x[0,1,:]}")
        return x
