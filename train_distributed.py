...
def print_model_details(config,model):
    if config['model.arch'] == 'rnn':
        ...
        print(f"num_of_rnn_layers={model.num_of_rnn_layers} model_count={model.model_count}")
        print(f"##############################################")
...
if __name__ == "__main__":
    ...
    while True:
        if data_seed_is_not_good:
          ...
          model, _ = get_model(config=config,model_count=cur_model_count,device=device,scale=scale_index)
        ...
        while perfect_model_count < target_model_count_subrun:
            ...
            with torch.no_grad():
                ...
                # DEBUG-start
                # if config['model.arch'] == 'rnn':
                #     test_accuracies_str += f"scale={scale_index} num_of_bad_models={model.DEBUG_get_number_of_bad_models(np.array(output.cpu()))} num_of_perfect_models={torch.count_nonzero(train_acc==1)}\n"
                #     scale_index += 1
                ...
                #         exit()
                # DEBUG-end 
                print(f"$$$$$$$$$$$$$$$$$$$$$ number of models with train_acc of '1' : {torch.count_nonzero(train_acc==1)} $$$$$$$$$$$$$$$$$$$$$")
                print(f"DEBUG: train_loss.shape={train_loss.shape} train_acc.shape={train_acc.shape} train_labels.shape={train_labels.shape}")
                ...
                if train_acc.max().detach().cpu().item() == 1:
                    print("DEBUG:############################################################# train_acc.max=1 :############################################################# ")
        ...
        if data_seed_is_not_good:
            ...
            new_models, kwargs  = get_model(arch=config['model.arch'],config=config,model_count=target_model_count_subrun,device="cpu",scale=scale_index)
