...
Section("model", "Model architecture parameters").params(
    ...
    init=Param(str, OneOf(("uniform", "regular", "uniform2", "uniform5", "sphere100", "sphere200","rnn")), default="uniform")
)
...
@section('model')
@param('arch')
def get_model(arch, config, model_count, device , scale=1):
    if arch == "mlp":
        kwargs = {"input_dim": 2,"output_dim": 2,"layers": config['model.mlp.layers'],"hidden_units": config['model.mlp.hidden_units'],"model_count": model_count}
        model = MLPModels(**kwargs,device=device)
       
    elif arch == "lenet":
        kwargs = {"output_dim": config['dataset.mnistcifar.num_classes'], "width_factor": config['model.lenet.width'], "model_count": model_count, 
        ...
        kwargs = {"input_dim": input_dim, "output_dim": config['dataset.mnistcifar.num_classes'], "model_count": model_count}
        model = LinearModels(**kwargs, device=device).to(device)
        
    elif arch == "rnn":
        kwargs = {"N": config['model.rnn.N'], "H_in": config['model.rnn.H_in'], "H_out": config['model.rnn.H_out'], "output_dim": config['dataset.mnistcifar.num_classes'], "embedding_size": config['model.rnn.embedding_size'],
        "complex": config['model.rnn.complex'],"diagonal": config['model.rnn.diagonal'], "linear_recurrent": config['model.rnn.linear_recurrent'], "embeddings_type": config['model.rnn.embeddings_type'],"enable_forward_normalize": config['model.rnn.enable_forward_normalize'],
        "num_of_rnn_layers": config['model.rnn.num_of_rnn_layers'], "model_count": model_count, "scale": scale}
        model = RNNModels(**kwargs)
        
    return model, kwargs


def reinitialize_modle(model,config,scale):
  ...
  if config['model.init'] == "uniform":
      ...
      print(f"DEBUG:reinitialize rnn model. scale={scale}")
      model.reinitialize_params(scale)
...
def build_results_directory_path(config, training_seed, data_seed, cur_num_samples, results_directory_name):
    ...
    if config['model.arch'] == 'rnn':
        ...
        output_path += f"{config['model.rnn.embeddings_type']}_embeddings_{forward_normalized_message}/"
    ...
    elif config['model.arch'] == 'rnn':
        output_path += f"rnn_N{config['model.rnn.N']}"\
                    f"H_in{config['model.rnn.H_in']}_"\
                    f"H_out{config['model.rnn.H_out']}_"\
                    f"embedding_size{config['model.rnn.embedding_size']}_"\
                    f"num_of_rnn_layers{config['model.rnn.num_of_rnn_layers']}_"
...
def print_and_save_loss_histogram(config, training_seed, loss_bin, data_seed, cur_num_samples, train_losses):
  #os.makedirs(os.path.join(config['output.folder'], "loss_histograms"), exist_ok=True)
  output_path = build_results_directory_path(config, training_seed, data_seed, cur_num_samples, "loss_histograms") + ".png"
  # bins = [i / 20 for i in range(21)] # lenet model
  bins = [i / 10 for i in range(0,100,5)] # rnn model
  print(f"bins:{bins}")
...
def print_model_details(config,model):
    if config['model.arch'] == 'rnn':
        ...
        print(f"N={model.N} H_in={model.H_in} H_out={model.H_out} embedding_size={embedding_size}")
        print(f"complex_model={model.complex} diagonal_model={model.complex} linear_recurrent={model.linear_recurrent} embeddings_type={model.embeddings_type} enable_forward_normalize={model.enable_forward_normalize}")
        print(f"num_of_rnn_layers={model.num_of_rnn_layers} model_count={model.model_count} target_model_count_subrun={config['distributed.target_model_count_subrun']} target_model_count={config['output.target_model_count']}")
        print(f"##############################################")
...
def DEBUG_compare_models_and_print_result(model1,model2,models_indices_to_compare):
    ...
    if model1.embeddings_type != "none":
        ...
        for param_idx in range(len(model1.encoder_layer_params)):
            if not np.array_equal(model1.encoder_layer_params[param_idx][:,models_indices_to_compare,:,:],model2.encoder_layer_params[param_idx]):
    ...
    for layer_idx in range(len(model1.rnn_layers_params)):
        for param_idx in range(len(model1.rnn_layers_params[layer_idx])):
            if not np.array_equal(model1.rnn_layers_params[layer_idx][param_idx][:,models_indices_to_compare,:,:],model2.rnn_layers_params[layer_idx][param_idx]):
...
if __name__ == "__main__":
    ...
    while True:
        if data_seed_is_not_good:
          ...
          print(f"%%%%%%%%%%%%%%%%%%%%% DEBUG:Main: train_data.shape={train_data.shape} test_all_data.shape={test_all_data.shape} %%%%%%%%%%%%%%%%%%%%%")
          torch.manual_seed(training_seed)
          scale = 8
          model, _ = get_model(config=config,model_count=cur_model_count,device=device,scale=scale)
          perfect_model_count = 0
          perfect_model_weights = []
          #target_model_count_subrun = config['distributed.target_model_count_subrun']
          target_model_count_subrun = 2000000 # for DEBUG only
          start_time = time.time()
          tested_model_count = 0
          loss_func = nn.CrossEntropyLoss(reduction='none')
          prior_max = 0
          DEBUG_max_loss = 0
          DEBUG_min_loss = float('inf')
          num_of_data_seed_attempts = 1
          cur_train_losses = []
        ...
        ### DEBUG - start ###
        #scales = [0.1,0.2,0.4,0.6,0.8,1,2,4,6,8,10,15,20,25,30,35,40,45,50,55]
        ...
        while perfect_model_count < target_model_count_subrun:
            ...
            if tested_model_count >= model_count_thresh_for_changing_data_seed:
              ...
              break
            reinitialize_modle(model,config,scale)
            ### DEBUG - start ###
            # if config['model.arch'] == 'rnn':
            #     print("printing model params after initialization:")
            #     model.DEBUG_print_model_params(model_num=0,[0])
            #     model.DEBUG_compare_models_params(0,1)
             ### DEBUG - end ###
           
            ### DEBUG - start ###
            if not print_reinitialization_time:
                DEBUG_time_1 = time.time()
                print_reinitialization_time = True
            else:
                reinitialization_time = time.time() - DEBUG_time_1
                print(f"%%%%%%%%%%%%%%%%%%%%%%DEBUG: reinitialize_modle time is:{reinitialization_time} seconds%%%%%%%%%%%%%%%%%%%%%%")
                DEBUG_time_1 = time.time()
            ### DEBUG - end ###
            ...
            with torch.no_grad():
                ...
                print(f"DEBUG: train_acc.shape={train_acc.shape}")
                #print(f"DEBUG: train_acc.shape={train_acc.shape}")
                
            #DEBUG-start
            print(f"DEBUG: output.shape={output.shape}")
            perfect_model_idxs = (train_acc == 1.0).nonzero().squeeze(1)
            # print(f"DEBUG: perfect_model_idxs.shape={perfect_model_idxs.shape}")
            # print(f"DEBUG: perfect_model_idxs.shape[0]={perfect_model_idxs.shape[0]}")
            num_of_perfect_models = perfect_model_idxs.shape[0]
            #non_zero_perfect_model_idxs = perfect_model_idxs.nonzero()
            print(f"DEBUG: number of models with (train_acc == 1.0) : {num_of_perfect_models}")
            if config['model.arch'] == 'rnn' and num_of_perfect_models < 1: # No reason to calculate test_acc if number of perfect_models is so small
                number_of_bad_models = model.DEBUG_get_number_of_bad_models(np.array(output.cpu()))
                test_accuracies_str += f"scale={scale} num_of_bad_models={number_of_bad_models} num_of_perfect_models=0 num_of_perfect_models_with_good_test_acc=0 \n"
                print(f"DEBUG: scale={scale} num_of_bad_models={number_of_bad_models} num_of_perfect_models=0 num_of_perfect_models_with_good_test_acc=0")
                print("#"*60)
                scale_index += 1
                if scale_index > len(scales)-1:
                    output_path = build_results_directory_path(config, training_seed, data_seed, cur_num_samples, "scale_finding") + ".txt"
                    with open(output_path,'w') as file:
                        file.write(test_accuracies_str)
                    print("Test Done")
                    exit()
                else:
                    continue
            if config['model.arch'] != 'rnn':
                print("entered cuda0 condition")
                number_of_models_that_fit_in_gpu_memory = 200
                number_of_perfect_model_with_good_test_acc_idxs = 0
                for i in range(0, num_of_perfect_models, number_of_models_that_fit_in_gpu_memory):
                    perfect_model_weights = []
                    curr_perfect_model_idxs = perfect_model_idxs[i:min(i+number_of_models_that_fit_in_gpu_memory, num_of_perfect_models)]
                    curr_num_of_perfect_models = curr_perfect_model_idxs.shape[0]
                    perfect_model_weights.append(model_result.get_weights_by_idx(curr_perfect_model_idxs)) # perfect_model_weights is a dictionary of weights
                    good_models_state_dict = dict()
                    cat_dim = 1 if config['model.arch'] in ["mlp", "linear", "rnn"] else 0
                    for k in perfect_model_weights[0].keys():
                        if config['model.arch'] in ["mlp", "linear", "lenet"]:
                    ...
                    perfect_models, kwargs  = get_model(arch=config['model.arch'],config=config,model_count=curr_num_of_perfect_models,device="cpu",scale=scale)
                    perfect_models.load_state_dict(good_models_state_dict)
                    #print(f"DEBUG: perfect_models size:{sys.getsizeof(perfect_models)} model_result size:{sys.getsizeof(model_result)} test_all_data size:{sys.getsizeof(test_all_data)}")
                    print(f"DEBUG: before test_Acc calc. batch_size=256 curr_num_of_perfect_models={curr_num_of_perfect_models}")
                    test_loss, test_acc, _ = calculate_loss_acc(test_all_data.cpu(), test_all_labels.cpu(), perfect_models.forward, loss_func, batch_size=256)
                    print(f"DEBUG: after test_Acc calc. batch_size=256")
                    perfect_model_with_good_test_acc_idxs = (test_acc > 0.8)
                    number_of_perfect_model_with_good_test_acc_idxs += torch.count_nonzero(perfect_model_with_good_test_acc_idxs)
                    print(f"DEBUG: number of models that were tested so fat: {min(i+number_of_models_that_fit_in_gpu_memory, num_of_perfect_models)}")
                print(f"$$$$$$$$$$$$$$$$$$$$$ number of perfect_model_with_good_test_acc_idxs : {number_of_perfect_model_with_good_test_acc_idxs} $$$$$$$$$$$$$$$$$$$$$")
            else:
                print("entered else condition")
                perfect_model_weights.append(model_result.get_weights_by_idx(perfect_model_idxs)) # perfect_model_weights is a dictionary of weights
                good_models_state_dict = dict()
                cat_dim = 1 if config['model.arch'] in ["mlp", "linear", "rnn"] else 0
                for k in perfect_model_weights[0].keys():
                    if config['model.arch'] in ["mlp", "linear", "lenet"]:
                        ...
                        good_models_state_dict[k] = np.concatenate([weights_dictionary[k] for weights_dictionary in perfect_model_weights], axis=cat_dim)
                # print(f"DEBUG: good_models_state_dict[rnn_layer0_param0].shape={good_models_state_dict['rnn_layer0_param0'].shape}")
                # print(f"DEBUG: model_result.rnn_layers_params[0][0].shape={model_result.rnn_layers_params[0][0].shape}")
                perfect_models, kwargs  = get_model(arch=config['model.arch'],config=config,model_count=num_of_perfect_models,device="cpu",scale=scale)
                perfect_models.load_state_dict(good_models_state_dict)
                #print(f"DEBUG: perfect_models size:{sys.getsizeof(perfect_models)} model_result size:{sys.getsizeof(model_result)} test_all_data size:{sys.getsizeof(test_all_data)}")
                if config['model.arch'] == 'rnn':
                    DEBUG_compare_models_and_print_result(model_result,perfect_models,perfect_model_idxs.tolist())
                print("DEBUG: before test_Acc calc. batch_size=1")
                test_loss, test_acc, _ = calculate_loss_acc(test_all_data.cpu(), test_all_labels.cpu(), perfect_models.forward, loss_func, batch_size=1)
                print(f"DEBUG: after test_Acc calc. batch_size=1")
                perfect_model_with_good_test_acc_idxs = (test_acc > 0.8)
                print(f"$$$$$$$$$$$$$$$$$$$$$ number of perfect_model_with_good_test_acc_idxs : {torch.count_nonzero(perfect_model_with_good_test_acc_idxs)} $$$$$$$$$$$$$$$$$$$$$")
            #DEBUG-end
                
            #DEBUG-start
            #rint(f"DEBUG: train_loss.shape={train_loss.shape} train_acc.shape={train_acc.shape} train_labels.shape={train_labels.shape}")
            #print(f"DEBUG: train_labels={train_labels}")
            #print(f"DEBUG: train_loss={train_loss} train_acc={train_acc} min_train_loss = {train_loss.min()} max_train_acc={train_acc.max()}")
            #DEBUG-end
            ...
            if train_loss.min() < DEBUG_min_loss:
                DEBUG_min_loss = train_loss.min()
            if train_loss.max() > DEBUG_max_loss:
                DEBUG_max_loss = train_loss.max()
            #print(f"DEBUG: tested_model_count={tested_model_count} min_train_loss={DEBUG_min_loss} max_train_loss={DEBUG_max_loss} max_train_acc={prior_max}")
            #print("tested_model_count", tested_model_count)
            # DEBUG-start
            if config['model.arch'] == 'rnn':
                number_of_bad_models = model.DEBUG_get_number_of_bad_models(np.array(output.cpu()))
                test_accuracies_str += f"scale={scale} num_of_bad_models={number_of_bad_models} num_of_perfect_models={num_of_perfect_models} num_of_perfect_models_with_good_test_acc={torch.count_nonzero(perfect_model_with_good_test_acc_idxs)}\n"
                print(f"DEBUG: scale={scale} num_of_bad_models={number_of_bad_models} num_of_perfect_models={num_of_perfect_models} num_of_perfect_models_with_good_test_acc={torch.count_nonzero(perfect_model_with_good_test_acc_idxs)}")
                print("#"*60)
                scale_index += 1
                if scale_index > len(scales)-1:
                    #os.makedirs(os.path.join(config['output.folder'], "scale_finding"), exist_ok=True)
                    output_path = build_results_directory_path(config, training_seed, data_seed, cur_num_samples, "scale_finding") + ".txt"
                    with open(output_path,'w') as file:
                        file.write(test_accuracies_str)
                    print("Test Done")
            ...
            #perfect_model_idxs = (train_acc == 1.0)
            ...
            if perfect_model_idxs.sum() > 0:
                if perfect_model_count > target_model_count_subrun:
                    ...
                    print(f"DEBUG: perfect_model_idxs.shape={perfect_model_idxs.shape} perfect_model_idxs.nonzero().squeeze(1).shape={perfect_model_idxs.nonzero().squeeze(1).shape}")
        ...
        if data_seed_is_not_good:
            if num_of_data_seed_attempts < max_data_seed_attemps:
             print(f"Trying a new data seed - only {perfect_model_count} out of {target_model_count_subrun} perfect models were found for set up: cur_num_samples:{cur_num_samples} cur_loss_bin:{cur_loss_bin} with data_seed:{data_seed}")
              ...
              if plot_loss_histogram_dict[cur_num_samples]: # we want to plot only one histogram per 'cur_num_samples'
                print_and_save_loss_histogram(config, training_seed, cur_loss_bin, data_seed, cur_num_samples, cur_train_losses)
            ...
            new_models, kwargs  = get_model(arch=config['model.arch'],config=config,model_count=target_model_count_subrun,device="cpu",scale=scale)
            ...
            with torch.no_grad():
                ...
                test_loss, test_acc, _ = calculate_loss_acc(test_all_data.cpu(), test_all_labels.cpu(), new_models.forward, loss_func, batch_size=1) # we don't use forward_normalize here because we are only using test accuracy, where normalization doesn't have any impact.
            ...
            output_path = build_results_directory_path(config, training_seed, data_seed, cur_num_samples, "models")
            ...
            update_model_stats_table(
                db_path, 
                model_id=model_id, data_seed=original_data_seed, 
    ...
    print_stds_of_avg_acuuracies(db_path)
