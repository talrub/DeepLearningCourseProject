...
def rnn_forward_pass(params, complex, diagonal ,linear_recurrent, embds): 
    ...
    embedding_size = embds.shape[2] # embds.shape= (num_of_examples,1,embeddings_size,1) without embeddings / (num_of_examples,model_count,embeddings_size,1) with embeddings
    
    #print(f"rnn_forward_pass: embds.shape={embds.shape}")
    H_in = B_re.shape[3]
    if complex:
        M = A_re + 1j*A_im
        B = B_re + 1j*B_im
    ...
    if linear_recurrent:
        ...
        embds = np.reshape(embds,(embds.shape[0],embds.shape[1],int(embedding_size/H_in),H_in)) # (num_of_examples,1,784,1) without embeddings / (num_of_examples,model_count,784,1) with embeddings
        #print(f"rnn_forward_pass: after reshape embds.shape={embds.shape}")
        for i in range(embds.shape[2]): # Going over all the rows of the examples
            input = embds[:,:,i,:] # input.shape=(num_of_examples,1,1) without embeddings / (num_of_examples,model_count,1) with embeddings
            input = np.expand_dims(input,axis=(3)) # input.shape=(num_of_examples,1,1,1) without embeddings / (num_of_examples,model_count,1,1) with embeddings
            #print(f"DEBUG: rnn_forward_pass: input.shape={input.shape}")
            B_mul_input = B@input
            M_mul_x = M@x
...
class RNNModels:
    def __init__(self,N, H_in, H_out, output_dim, embedding_size, complex, diagonal, linear_recurrent, embeddings_type, enable_forward_normalize, num_of_rnn_layers, model_count, scale):
        self.N = N
        self.H_in = H_in
    ...
    def weight_normalization(self):
        if self.embeddings_type != "none":
            ...
            W, b = self.encoder_layer_params
            W_norm = np.linalg.norm(W, axis=(2, 3), keepdims=True)
            encoder_layer_normalized_params_list.append(W/W_norm)
            encoder_layer_normalized_params_list.append(b) # bias is not normelized
