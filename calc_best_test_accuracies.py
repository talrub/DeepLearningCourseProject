...
def rows_to_dataframe(rows):
    columns = ['num_train_samples', 'loss_bin_l', 'loss_bin_u', 'SUM(perfect_model_count)', 'AVG(test_acc)', 'AVG(train_time)']
...
#                 "scale_0.1_N_32_edo_init_Inefficient_forward_pass_linear_recurrent_complex_diag_real_im_parametrization_rnn_same_seeds",
...
#                 "scale_0.1_N_32_edo_init_Inefficient_forward_pass_linear_recurrent_complex_diag_stable_ring_init_parametrization_r_min_0.0_r_max_1.0_max_phase_6.28_rnn_same_seeds",
#                 "scale_0.1_N_32_edo_init_Inefficient_forward_pass_linear_recurrent_complex_diag_stable_ring_init_parametrization_r_min_0.0_r_max_1.0_max_phase_6.28_gamma_normalization_rnn_same_seeds"]
# scale_0.1_N_64
...
for model_name in models_names:
    ...
    arch_df_dict[model_name] = best_test_accuracy_per_num_train_sample_df

columns = ['num_train_samples','arch','best_AVG(test_acc)']
best_test_accuracies_summary_df = pd.DataFrame(columns=columns)
for num_train_samples in num_train_samples_list:
    for model_name in models_names:
        #print(f"DEBUG: num_train_samples={num_train_samples} model_name={model_name}")
        curr_df = arch_df_dict[model_name]
        #print(f"curr_df")
        #print(curr_df)
        if not curr_df.empty and (num_train_samples in curr_df['num_train_samples'].values):
            best_test_acc_df = curr_df.loc[(curr_df['num_train_samples'] == num_train_samples)].reset_index()
            best_test_acc = best_test_acc_df.loc[0, "AVG(test_acc)"]
        else:
            best_test_acc = -1

        #print(f"DEBUG: after")
        data_point = {'num_train_samples': [num_train_samples], 'arch': [model_name],'best_AVG(test_acc)': [best_test_acc]}
        data_point_df = pd.DataFrame(data_point)
        best_test_accuracies_summary_df = pd.concat([best_test_accuracies_summary_df,data_point_df],ignore_index=True)
...
excel_file_path = f'output/table2/mnist_guess_rnn/best_test_acc_summary/{experiments_prefix}.xlsx'
print(f"DEBUG:excel_file_path={excel_file_path}")
print("Best test accuracies summary:")
print(best_test_accuracies_summary_df)
best_test_accuracies_summary_df.to_excel(excel_file_path, sheet_name='best_test_accuracies_summary', index=False)
